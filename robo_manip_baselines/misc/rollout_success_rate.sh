...
Usage:
  rollout_success_rate.sh \
    ...
    [--duration 30] \
    [--task-desc "..." ] \
    [--task-desc-map-file path/to/task_desc_map.txt] \
    [--mujoco-gl egl|osmesa|glfw] \
    [--pyopengl-platform egl|osmesa|glx] \
    [--stop-on-error] \
    [--python python] \
...
task-desc map file format:
  One "ENV<TAB>DESC" per line. Lines starting with # are ignored.
  Example:
    MujocoHsrShelfPaP\tPick and place item on shelf
    MujocoHsrDoor\tOpen the door
EOF
}
...
# Backward-compatible behavior: if --env receives a space-separated list,
# treat it as --env-list when --env-list is not provided.
if [[ -n "$ENV_NAME" && -z "$ENV_LIST_STR" && "$ENV_NAME" == *" "* ]]; then
  ENV_LIST_STR="$ENV_NAME"
  ENV_NAME=""
fi
...
if [[ -n "$TASK_DESC_MAP_FILE" ]] && [[ ! -f "$TASK_DESC_MAP_FILE" ]]; then
  echo "[ERROR] task-desc map file not found: $TASK_DESC_MAP_FILE" >&2
  exit 1
fi

if [[ -n "$MUJOCO_GL_BACKEND" ]]; then
  export MUJOCO_GL="$MUJOCO_GL_BACKEND"
fi
if [[ -n "$PYOPENGL_PLATFORM_BACKEND" ]]; then
  export PYOPENGL_PLATFORM="$PYOPENGL_PLATFORM_BACKEND"
fi

# Headless default: prefer EGL on compute nodes without DISPLAY.
if [[ -z "${DISPLAY:-}" ]]; then
  if [[ -z "${MUJOCO_GL:-}" ]]; then
...
if [[ -n "$TASK_DESC_MAP_FILE" ]]; then
  while IFS= read -r raw_line || [[ -n "${raw_line:-}" ]]; do
    ...
    if [[ "$line" == *$'\t'* ]]; then
      ...
      # Fallback: split at first whitespace sequence
      env_key="${line%%[[:space:]]*}"
      desc="${line#${env_key}}"
    fi
    ...
    if [[ -z "$env_key" || -z "$desc" ]]; then
      continue
    fi
    TASK_DESC_BY_ENV["$env_key"]="$desc"
  done < "$TASK_DESC_MAP_FILE"
fi
...

{
  ...
  echo "[INFO] task_desc_default=${TASK_DESC:-<none>}"
  echo "[INFO] task_desc_map_file=${TASK_DESC_MAP_FILE:-<none>}"
  echo "[INFO] MUJOCO_GL=${MUJOCO_GL:-<unset>}"
  echo "[INFO] PYOPENGL_PLATFORM=${PYOPENGL_PLATFORM:-<unset>}"
  echo "[INFO] stop_on_error=$STOP_ON_ERROR"
  echo "[INFO] output_dir=$OUTPUT_DIR"

  for env in "${ENV_LIST[@]}"; do
    env_task_desc="${TASK_DESC_BY_ENV[$env]:-$TASK_DESC}"
    echo "[INFO] ===== env=${env} ====="
    echo "[INFO] task_desc=${env_task_desc:-<none>}"
    ...
    for ((rep=1; rep<=NUM_ROLLOUTS; rep++)); do
      ...
      for world_idx in "${WORLD_IDX_LIST[@]}"; do
        ...
        if [[ -n "$env_task_desc" ]]; then
          cmd+=(--task_desc "$env_task_desc")
        fi
        ...
        if [[ "$cmd_status" -ne 0 ]]; then
          ...
          if [[ "$STOP_ON_ERROR" -eq 1 ]]; then
            echo "[ERROR] Stopping early due to rollout command failure."
            exit "$cmd_status"
          fi
        fi
